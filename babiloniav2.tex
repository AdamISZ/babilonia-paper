...
\subsection{Bet amount hiding}
\label{sec:bamhide}

This is the principal difference between this version 2 of this paper, and version 1.

In the following, a naive implementation of the protocol will \emph{first} be described; it is valuable, but from the privacy perspective specifically, it has a weakness: it deals in payouts of a concrete size, which can be guessed and then confirmed by a blockchain analyst by comparing the pay-in amounts to the funding transaction, with the payout amounts in the payout transaction. In Section~\ref{sec:agg} we describe a technique to \emph{completely} remove this fingerprint; since we can make several aggregate bets taking an arbitrary (and not public) partition of the funding amount, we can make the relation between the payout and the payin amounts be any arbitrary value, thus being indistinguishable (in terms of bitcoin amount) from ordinary payments.

While this is a more complicated interaction between the participants offchain, it has no impact on the final onchain footprint in the normal case; only if one side disconnects must we publish a larger number of output transactions (this fact limits the partition to $N$ sub-bets where $N$ is realistically less than 10 and probably 5 or less for liquidity reasons; but the hiding effect of the secret partition already works with only \emph{two} sub-bets).

The best way to explain this slightly surprising fact: a single bet leaks the amount through the connection between the amounts in two transactions (Funding, Payout). A set of $N$ \underline{equal-sized} and then aggregated bets would still leak it to an adversary who simply guesses integer multiples. But a set of $N$ bets that arbitrarily partition the staked coins (e.g. take 1 and split into 0.21, 0.72, 0.07) result in payouts across a continuous distribution, because the partition fractions themselves can be a continuous variable.
...
The soundness of the proof is relatively simple; it's 2-special soundness in the ROM applied to the relevant hash function:
...
The single bet of \S\ref{sec:crypto}, even in its $1$-of-$N$ form, leaves one residual fingerprint that the payment-shaping of \S\ref{sec:pst} does \emph{not} remove: the \textbf{size of the winning payout}.

Let's take Figure~\ref{fig:sweetener} to illustrate the point. The adversary sees two inputs of 0.8 and 0.2. If it assumes that 0.24 is the payment out of this, and it guesses that this is following the Babilonia protocol as per this paper, it knows that 0.8 is the player. If the pot-sweetener can be both large and bi-directional between the participants, then the adversary is stuck in trying to deduce that the final output of 0.1 corresponds or not to 2x the pot contributions (it sees 0.24, compares with 0.2 and says to itself ``the stake from each party should be close to 0.04''). But realistically this $\epsilon$ should usually be a small fraction of the bet amount, or perhaps zero. If $\epsilon$ were actually zero, it would be trivial to relate: pot 0.25 is 0.05 greater than one of the inputs (Alice's) and the payout output 0.1 is exactly twice that.

If instead Alice and Bob did an $N:1$ \emph{equal} bet using the same tx structure, and no pot sweetener, then the final payout would not be $2 \times$ that implied funding transaction payment, but it might be $N \times$ for some $N$ that the adversary simply guesses by trial and error.

If Alice and Bob aggregated 10 such $N:1$ bets, it would be more difficult, but the same basic weakness persists: there is just a discrete distribution of outcomes, $N^k$ where $k$ is the number of bets. To make this infeasible to guess would require large numbers which goes counter to the fact that an aggregated bet may have to be de-aggregated onchain in a non-cooperative case.

Hence, this section describes an overlay --- already outlined conceptually in \S\ref{sec:bamhide}, and already implemented in the reference code \cite{babilonia} --- that removes this tell while keeping the two-transaction covert footprint intact. It introduces \textbf{no new cryptographic assumption}: it composes $k$ independent instances of the single-bet primitive of \S\ref{sec:crypto} and merges their outcomes with a cooperative novation that is exactly the \S\ref{sec:coop} overlay, generalized from one bet to $k$.
...
% ============ FIGURE 1: cooperative aggregate settlement ============
\begin{figure}[h]
\centering
\begin{tikzpicture}[>=Stealth,
  tx/.style   ={draw, rounded corners, align=center, inner sep=5pt, font=\footnotesize},
  pot/.style  ={draw, rounded corners, align=center, inner sep=5pt, font=\footnotesize, fill=green!12},
  sb/.style   ={draw, rounded corners, align=center, inner sep=4pt, font=\scriptsize, text width=2.9cm},
  win/.style  ={sb, fill=green!16, draw=green!55!black},
  lose/.style ={sb, fill=black!4},
  ann/.style  ={font=\scriptsize, text=black!60},
  grp/.style  ={draw, densely dotted, rounded corners, inner sep=3mm}]

\node[tx] (fund) at (0,0)
  {\textbf{FundingTx}\quad(2-in, 2-out)\\ $F_A + F_B \ \to\ U_1,\ \ \text{player change }c_B$};
\node[pot] (u1) at (0,-1.9)
  {\textbf{pot }$U_1 \ \ge\ \sum_{i=1}^{3} N_i s_i \ +\ \text{fee}$};

\node[win]  (s1) at (-4.6,-4.5) {\textbf{sub-bet 1}\\ $1$-of-$2$,\ stake $s_1$\\[1pt] \textbf{WON}\ $\Rightarrow\ 2s_1$};
\node[lose] (s2) at ( 0.0,-4.5) {\textbf{sub-bet 2}\\ $1$-of-$4$,\ stake $s_2$\\[1pt] \textbf{lost}\ $\Rightarrow\ 0$};
\node[win]  (s3) at ( 4.6,-4.5) {\textbf{sub-bet 3}\\ $1$-of-$3$,\ stake $s_3$\\[1pt] \textbf{WON}\ $\Rightarrow\ 3s_3$};
\node[grp, fit=(s1)(s2)(s3)] (box) {};
\node[ann, align=left, anchor=south west] at (box.north west) {$k=3$ sub-bets --- logical, resolved off-chain};

\node[tx, fill=green!8, text width=8.2cm] (agg) at (0,-7.6)
  {\textbf{Aggregate settlement}\quad(cooperative key-path spend, 1-in 2-out)\\[2pt]
   output 1 $\to$ player:\ \ $\mathrm{net}_B \;=\; 2s_1 + 3s_3$\\
   output 2 $\to$ dealer:\ \ $U_1 - \mathrm{net}_B - \text{fee}$};

\draw[->] (fund) -- (u1);
\draw[->, densely dotted] (u1) -- (box.north)
      node[midway, right, ann] {funds 3 sub-bets};
\draw[->, thick, green!55!black] (s1.south) -- (agg.north);
\draw[->, thick, green!55!black] (s3.south) -- (agg.north);
\draw[->, densely dotted] (s2.south) -- (agg.north);
\node[ann, anchor=west, align=left] at (2.7,-6.4) {co-signed as \emph{one}\\ key-path spend of $U_1$};
\end{tikzpicture}
\caption{\textbf{Aggregated bet --- cooperative settlement (happy path).} The $k=3$ sub-bets
(here $1$-of-$2$, $1$-of-$4$, $1$-of-$3$, with independent un-quantized stakes $s_1,s_2,s_3$)
are funded into one pot $U_1$ but resolved \emph{off-chain}. The parties then co-sign a
\emph{single} key-path spend of $U_1$ paying the player the net of the sub-bets he won,
$\mathrm{net}_B = 2s_1 + 3s_3$, and the balance to the dealer. On chain this is just two
ordinary payments (FundingTx, then a 1-in 2-out spend); the sub-bets leave no trace.}
\label{fig:agg}
\end{figure}


% ============ FIGURE 2: enforcement fallback ============
\begin{figure}[h]
\centering
\begin{tikzpicture}[>=Stealth,
  tx/.style   ={draw, rounded corners, align=center, inner sep=5pt, font=\footnotesize},
  pot/.style  ={draw, rounded corners, align=center, inner sep=5pt, font=\footnotesize, fill=green!12},
  spl/.style  ={draw, densely dashed, rounded corners, align=center, inner sep=5pt, font=\footnotesize, text width=8.4cm},
  sp/.style   ={draw, densely dashed, rounded corners, align=center, inner sep=4pt, font=\scriptsize, text width=3.2cm},
  set/.style  ={draw, densely dashed, rounded corners, align=center, inner sep=4pt, font=\scriptsize, text width=3.2cm},
  win/.style  ={fill=green!14, draw=green!55!black},
  lose/.style ={fill=black!4},
  ann/.style  ={font=\scriptsize, text=black!60}]

\node[tx] (fund) at (0,0)
  {\textbf{FundingTx}\quad(2-in, 2-out)\\ $F_A + F_B \ \to\ U_1,\ \ \text{player change }c_B$};
\node[pot] (u1) at (0,-1.9) {\textbf{pot }$U_1$};
\node[spl] (split) at (0,-3.9)
  {\textbf{Splitter}\quad(1-in, $k$-out)\\ $U_1 \ \to\ U^{(1)},\, U^{(2)},\, U^{(3)}$};

\node[sp] (p1) at (-4.6,-6.1) {sub-pot $U^{(1)} = 2 s_1$};
\node[sp] (p2) at ( 0.0,-6.1) {sub-pot $U^{(2)} = 4 s_2$};
\node[sp] (p3) at ( 4.6,-6.1) {sub-pot $U^{(3)} = 3 s_3$};

\node[set, win]  (o1) at (-4.6,-8.6) {\textbf{sub-bet 1: won}\\ player claims $2s_1$\\ (key-path, as \S\ref{sec:crypto})};
\node[set, lose] (o2) at ( 0.0,-8.6) {\textbf{sub-bet 2: lost}\\ dealer settles / refund\\ (as \S\ref{sec:crypto})};
\node[set, win]  (o3) at ( 4.6,-8.6) {\textbf{sub-bet 3: won}\\ player claims $3s_3$\\ (key-path, as \S\ref{sec:crypto})};

\draw[->] (fund) -- (u1);
\draw[->] (u1) -- (split);
\draw[->, densely dashed] (split.south) -- (p1.north);
\draw[->, densely dashed] (split.south) -- (p2.north);
\draw[->, densely dashed] (split.south) -- (p3.north);
\draw[->, densely dashed] (p1) -- (o1);
\draw[->, densely dashed] (p2) -- (o2);
\draw[->, densely dashed] (p3) -- (o3);
\end{tikzpicture}
\caption{\textbf{Aggregated bet --- enforcement fallback (used only if cooperation fails).}
The same pot $U_1$ is instead spent by the pre-signed \emph{splitter} into $k=3$ sub-pots,
each settled independently exactly as a stand-alone bet of \S\ref{sec:crypto}
(settlement/claim on a win, timeout refund otherwise). The player's total is identical to the
cooperative case, $2s_1 + 3s_3$, but the footprint is now $k{+}1$ transactions (plus
per-sub-bet claims/refunds) and the settlement scripts can surface on-chain. Because both
spends of $U_1$ yield the same net, no rational party ever takes this path.}
\label{fig:agg-fallback}
\end{figure}
