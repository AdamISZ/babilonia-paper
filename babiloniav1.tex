...
\date{July 12, 2026 \\ \medskip Version 1.0}
...
Next, to start the actual protocol, Alice randomly generates two secret values $a_1, a_2$ from the set of valid scalars in $\mathbb{F}_p$ where $p$ is the order of the secp256k1 group. She sends $A_i = a_i G,\quad i \in \{1,2\}$ to Bob along with a PoK of each of these secret discrete logs using the vanilla method (Schnorr signature). We call $A_i$ the ``thimbles''.
...
The value $W_b = w_b G$ is a \emph{fresh hidden claim key} chosen by Bob, secret to this transaction and never revealed to Alice. We fix the index convention used throughout: $c\in\{1,2\}$ is \emph{Alice's} secret choice, and $y\in\{1,2\}$ is \emph{Bob's} secret guess; Bob wins iff $y=c$. So $A_y$ is the thimble corresponding to Bob's guess. His inability to know Alice's choice $c$ is clearly not dependent on any assumption cryptographically, only on Alice's general security.
...
\label{def:piB}
Given public thimbles $A_1,A_2\in\mathbb{G}$ and Bob's claim key $K\in\mathbb{G}$, $\pi_B$ is a zero-knowledge proof for the relation
\[
\mathcal{R}_{\pi_B}=\Big\{\ \big((K,A_1,A_2);\,(w_b,y)\big)\ :\ y\in\{1,2\},\ \ K = w_b\,G + A_y\ \Big\}.
\]
Equivalently, Bob knows the discrete log of $K-A_y$ with respect to $G$ for one branch $y$ (his guess), \emph{without revealing which}. It is realised as a $1$-of-$2$ CDS disjunction \cite{cds} of Schnorr proofs of knowledge of $\mathrm{dlog}_G(K-A_1)$ or $\mathrm{dlog}_G(K-A_2)$: soundness forces $K$ to be $A_1$ or $A_2$ offset by a value Bob knows, and witness-indistinguishability hides $y$.
\end{definition}
...
\]
where $t_1$ is a relative timelock in blocks (BIP68). $t_1$ is small relative to $T_R$. Thus the honest winner spends $O_K$ by the \emph{key path} under $K$ (an ordinary taproot payment, no script revealed), while Alice can reclaim after $t_1$ blocks via this single script leaf only if Bob never claims.
...
\label{def:piA}
Fix a pad function $f:\mathbb{F}_p\to\mathbb{F}_p$ (the ``decryption'' map; concrete choices in Section~\ref{sec:pia}). Given public $(\textrm{ctxt},T,A_1,A_2)$ with $\textrm{ctxt}\in\mathbb{F}_p$ and $T,A_1,A_2\in\mathbb{G}$, $\pi_A$ is a zero-knowledge proof for
\[
\mathcal{R}_{\pi_A}=\Big\{\ \big((\textrm{ctxt},T,A_1,A_2);\,(t,c,a_c)\big)\ :\ c\in\{1,2\},\ \ T=tG,\ \ A_c=a_c G,\ \ \textrm{ctxt}=a_c+f(t)\ \Big\}.
\]
In words: the adaptor point $T$ has discrete log $t$; the ciphertext decrypts under the pad to a scalar $a_c$ whose public image $A_c$ is one of the two published thimbles; and the branch $c$ (hence the outcome) is hidden.
\end{definition}
...
\begin{enumerate}
\item Dealer sends funding information: $u_A$ for funding FundingTx, $P_A$ for MuSig2 destination key $M$ (will be outpoint $u_1$), receiving address for RefundTx $R_A$, change address for PayoutTx $C_A$, coop-overlay address $O_A$, nonces for RefundTx and PayoutTx and CoopOverlay signature negotiation. Simultaneously, she sends $A_1, A_2$ and a proof of knowledge of the dlogs $a_1, a_2$, keeping the latter secret.
\item Player validates the PoK of $A_1, A_2$. Player sends funding information: $u_B$ for funding FundingTx, $P_B$ for MuSig2 destination key $M$, (will be outpoint $u_1$), receiving address for RefundTx $R_B$, change address for PayoutTx $C_B$, nonces for RefundTx and PayoutTx and CoopOverlay signature negotiation, and $K$ for settlement transaction, along with a ZKP $\pi_B$ proving that $K$ is of the form $A_y + W_b$ for his choice $y$ from $A_1, A_2$ and a further key he proves knowledge of (see above for details).
...
\item Player calculates MuSig2 destination address $M$ and outpoint $u_1$, and full FundingTx ($u_A, u_B \rightarrow u_1, C_B$). Player then calculates RefundTx ($u_1 \rightarrow R_A, R_B$). Having received $\sigma_{A,R}$ he now calculates his own partial sig on RefundTx $\sigma_{B,R}$. Sends $\sigma_{B,R}$.
\item Dealer validates RefundTx with the given full MuSig2 signature $\sigma_{A,R} + \sigma_{B,R}$. If all validates, she signs the FundingTx (which she already calculated) on $u_A$ to get $\sigma{A, F}$ and passes this as a PSBT to Player.
\item Player validates RefundTx with the given full MuSig2 signature $\sigma_{A,R} + \sigma{B,R}$. If all validates, he co-signs the FundingTx (which he already calculated) on $u_B$ to get $\sigma{B, F}$ and combines this with the PSBT to broadcast the FundingTx.
...
\item Dealer calculates the adaptor signature and point $\sigma'_{A, P}, T$ for PayoutTx signing on $u_1$ (noting that the nonces were already exchanged for this MuSig session).   She then calculates $\pi_A$ and ciphertext $\textrm{ctxt}$ as described above and sends $\sigma'_{A, P}, T, \pi_A, \textrm{ctxt}$. $T$ was chosen so that $\textrm{ctxt} = a_c + t^2\ \textrm{mod}\ p$ and $tG = T$ and $a_c \in a_1,a_2$ and $\pi_A$ proves this.
...
\item Player sees $t$, and uses it to calculate $a_c = \textrm{ctxt} - t^2 \ \textrm{mod}\ p$. And then compares $a_c G$ with $A_y$, if they match, he has won and will calculate the full MuSig2 cosignature on PayoutTx as $\sigma{B,P} + \sigma'{A, P} + t$ and use it to broadcast PayoutTx. And must send another payment from $O_K$ to his own wallet independently before the timeout clause $t_1$.
...
\subsection{Extension to other payout probabilities}

We omit this here, and claim it is a trivial extension, though not one without limitations: instead of an OR condition over two possibilities, make it an OR condition over $N$ possibilities. This can give Bob a $p=1/n$ chance of success. (Probably $k/N$ for some threshold $k$ is also possible, but more complex). Worthy of note is that the simple sigma protocols shown here have linear scaling in communication over $N$, so this would be very non-ideal for large $N$, but SNARKs used as alternative proving mechanisms could overcome this. The earlier mentioned GTT work \cite{gerhardt2026} uses an OPRF mechanism which also removes this $O(N)$ scaling.
...
\subsubsection{Squaring proof security}
